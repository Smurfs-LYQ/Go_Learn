...
8. > 类型断言
	...
	- 用于if判断的
		...
		其中:
			...
			2. T : 表示断言"x"可能是类型
		```
		- 该语法返回两个参数，第一个参数是`x`转换为`T`类型后的变量，第二个值是一个布尔值，若为`true`则表示断言成功，为`false`则表示断言失败
   	- 用于switch判断的
		```
		语法格式:
			x.(type)
		...
		```
		该语法也返回两个参数，第一个参数是`x`转换为`T`类型后的变量，第二个值是一个布尔值，若为`true`则表示断言成功，为`false`则表示断言失败，但是第二个参数被switch接收了，所以不用额外命名
9. > 值接收者和指针接收者实现接口的区别
